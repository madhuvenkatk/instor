...
const Shopcart = () => {
    ...
    const removerow = (e) => {
        ...
        existing.splice(e.target.key,1);
    ...
    <table>
        <tbody>
        {products_arr.map((data) =>	(
 
        <tr>
             <td class="remove" key={this}>                 
        <button type="button" key={this} onClick ={item.id}>Delete</button>
    </td>
            <td class='ctd' width="70%">{data.title}</td>
            <td class='ntd' width="20%">{data.qty}</td>
    ...
    <a href="/about"><button class="btn_green">Proceed to Checkout</button></a>
    </div>
    </div>
    </div>
